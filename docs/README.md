...
23. [Stage 6.8/6.9/6.10/6.10.2 compaction corpus coverage and budget alignment](compaction-stage68-coverage.md) — coverage/runner hardening (13/14 v3 accepted, c_macro confirmed), Stage 6.9 context-aligned budget resolver, Stage 6.10 v3 default + autocompact-buffer policy, Stage 6.10.1 OpenAI masquerade + remote compaction endpoint, Stage 6.10.2 config path cleanup + backend auto-resolution (18 new tests).
...
| Compaction coverage | [Stage 6.8/6.9/6.10/6.10.2 compaction corpus coverage and budget alignment](compaction-stage68-coverage.md) | Stage 6.8: targeted file selection, full artifacts, 13/14 v3, c_macro confirmed. Stage 6.9: context-aligned budget resolver. Stage 6.10: v3 default, autocompact-buffer policy, model_auto_compact_token_limit, 4/4 v3 deep-coverage, 20 new tests. Stage 6.10.1: OpenAI masquerade, remote compaction endpoint, 14 new tests. Stage 6.10.2: config path cleanup, backend auto-resolution, 18 new tests. |
