...
| Manual recovery policy | [Backend manual recovery endpoint policy](backend-manual-recovery-endpoint-policy.md) | Design policy for future `/qz/recovery/plan` and `/qz/recovery/trigger` endpoints: allowed actions, forbidden actions, backoff schedule, authority flags, active request safety, telemetry events, HTTP semantics, and implementation slices 6–10. |
