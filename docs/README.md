...
20. [Compaction Codex setup](compaction-codex-setup.md) — how to wire `experimental_compact_prompt_file`, use `compaction.json`, keep env overrides explicit, and run manual smoke checks. Stage 6.10.1: OpenAI masquerade active, `/v1/responses/compact` remote compaction endpoint, double-compaction safety notes.
21. [Compaction live dogfood](compaction-live-dogfood.md) — Stage 6/6.1 direct-backend smoke runbook, live evidence, observed v2 fallback, and accepted opt-in v3 tuning. Stage 6.10.1: remote compaction smoke test runbook and Codex session verification.
...
23. [Stage 6.8/6.9/6.10/6.10.1 compaction corpus coverage and budget alignment](compaction-stage68-coverage.md) — coverage/runner hardening (13/14 v3 accepted, c_macro confirmed), Stage 6.9 context-aligned budget resolver, Stage 6.10 v3 default + autocompact-buffer policy, Stage 6.10.1 OpenAI masquerade + remote compaction endpoint (14 new tests).
...
| Compaction strategy | [Compaction audit and strategy](compaction-audit-and-strategy.md) | Codex audit (SHA 46f30d0), QuantZhai current behaviour, #8 promoted ideas, anchored schema v0, prompt v0, staged plan Stages 0–6.10.1. |
...
| Compaction setup | [Compaction Codex setup](compaction-codex-setup.md) | Wire `experimental_compact_prompt_file`, use `compaction.json`, preserve safe defaults, and run manual /compact smoke checks. Stage 6.10.1: OpenAI masquerade, `/v1/responses/compact`, double-compaction safety. |
| Compaction dogfood | [Compaction live dogfood](compaction-live-dogfood.md) | Stage 6/6.1/6.2/6.3/6.5/6.7 live smoke and extended dogfood notes: direct backend runbook, v3 acceptance/fallback evidence, latency baseline, corpus staging harness, multi-repo anti-overfit. Stage 6.10.1: remote compaction smoke runbook and Codex session verification. |
| Compaction corpus | [Compaction corpus dogfood](compaction-corpus-dogfood.md) | Stage 6.4/6.5/6.7/6.8: multi-repo staging harness for anti-overfit compaction testing. 8 repos, 4 scripts, 91 tests. |
...
| Compaction coverage | [Stage 6.8/6.9/6.10/6.10.1 compaction corpus coverage and budget alignment](compaction-stage68-coverage.md) | Stage 6.8: targeted file selection, full artifacts, 13/14 v3, c_macro confirmed. Stage 6.9: context-aligned budget resolver. Stage 6.10: v3 default, autocompact-buffer policy, model_auto_compact_token_limit, 4/4 v3 deep-coverage, 20 new tests. Stage 6.10.1: OpenAI masquerade, remote compaction endpoint, 14 new tests. |
...
docs/compaction-stage68-coverage.md
