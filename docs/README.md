...
8m. [Client-facing availability and 503 minimisation audit](client-facing-availability-and-503-minimisation-audit.md) — source-grounded audit of all 503-producing surfaces, Codex retry/error-mapping evidence, failure classes, better-than-503 options, and top implementation priorities. (2026-05-28)
...
| Compaction setup | [Compaction Codex setup](compaction-codex-setup.md) | Wire `experimental_compact_prompt_file`, use `compaction.json`, preserve safe defaults, and run manual /compact smoke checks. Stage 6.10.2: CODEX_HOME path corrected (`$HOME/.qz-codex/codex-home/config.toml`), compactor backend auto-resolution, 7-step smoke runbook. Stage 6.10.1: OpenAI masquerade, `/v1/responses/compact`, double-compaction safety. |
| Compaction dogfood | [Compaction live dogfood](compaction-live-dogfood.md) | Stage 6/6.1/6.2/6.3/6.5/6.7 live smoke and extended dogfood notes: direct backend runbook, v3 acceptance/fallback evidence, latency baseline, corpus staging harness, multi-repo anti-overfit. Stage 6.10.2: smoke runbook and config path corrected. Stage 6.10.1: remote compaction smoke runbook and Codex session verification. |
...
| Compaction coverage | [Stage 6.8/6.9/6.10/6.10.4 compaction corpus coverage and budget alignment](compaction-stage68-coverage.md) | Stage 6.8: targeted file selection, full artifacts, 13/14 v3, c_macro confirmed. Stage 6.9: context-aligned budget resolver. Stage 6.10: v3 default, autocompact-buffer policy, model_auto_compact_token_limit, 4/4 v3 deep-coverage, 20 new tests. Stage 6.10.1: OpenAI masquerade, remote compaction endpoint, 14 new tests. Stage 6.10.2: config path cleanup, backend auto-resolution, 18 new tests. Stage 6.10.3: remote compaction source selection fix, v3 fallback diagnostics, 8 new tests. Stage 6.10.4: context resolution fix (runtime_context_length field), 12 new tests. |
