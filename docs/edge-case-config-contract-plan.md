...
3. ~~**Slice C-design: First migration target selection** — COMPLETE~~
4. ~~**Slice C-impl: Physical move of A1** — COMPLETE (commit fb28945)~~
   - ~~`model_inventory_path()` returns `var/generated/model-inventory.json`~~
...
### Slice D-design (completed 2026-05-19) — A2/A3 coupled migration plan

**Status:** design only — no runtime implementation, no files moved.

---

#### 1. A2/A3 coupling audit

A2 and A3 are generated together by a single call:

```python
qz_codex_catalog.generate(inventory_path, catalog_dst, config_dst)
```

- `catalog_dst` = A2 path — the Codex model catalog JSON
- `config_dst` = A3 path — the Codex `config.toml` patched with `model_catalog_json = "<absolute A2 path>"`

**Why they must move together:**

1. `generate()` writes A2 first, then patches A3 to embed the absolute path of A2:
   ```python
   catalog_line = f'model_catalog_json = "{catalog_dst}"'
   config_dst.write_text(...)
   ```
2. If A2 moves but A3 stays, A3's `model_catalog_json` still points to the old A2 path — stale until next refresh.
3. If A3 moves but A2 stays, A3 is unreachable at old location and the new A3 references the old A2 path.
4. Moving both in one helper change + running refresh produces a consistent pair.

**Why A3 must not be removed in this slice:**
A3 (config.toml) is still generated by `generate()`. Removing A3 generation requires a separate decision and separate slice.
...
```
A2 (server Codex catalog JSON)
  Writer:   _refresh_codex_catalog() -> generate(..., catalog_dst, config_dst)
            qz_request_router.py:395-398
  Readers:
    /qz/codex/model-catalog endpoint — reads and serves A2 over HTTP
            qz_codex_client_config.py:codex_model_catalog_content()
    /qz/codex/client-config — reads A2 sha256/mtime metadata
            qz_codex_client_config.py:codex_client_config_payload()
    /qz/control-plane — reports A2 path/exists
            qz_control_plane.py:_codex_catalog_info()
    /qz/config/effective — reports A2 path, staleness vs A1
            qz_config_report.py:392, 445, 569-576
    /qz/models/refresh response — reports A2 path
            qz_request_router.py:1469

A3 (server Codex config.toml)
  Writer:   generate() patches config_dst in-place with model_catalog_json = "<A2 abs path>"
            qz_codex_catalog.py:298-303
  Readers:
    /qz/config/effective — reports A3 path, staleness vs A2
            qz_config_report.py:393, 446, 579-585
```

**No consumer of A2/A3 is the qz-codex client after #58.** The client downloads the catalog from `/qz/codex/model-catalog` (HTTP) and writes to its own local `CODEX_HOME` (`$HOME/.qz-codex/codex-home`). Server A2/A3 paths are purely server-internal.

---

#### 3. Proposed target paths

| Option | A2 | A3 | Note |
|---|---|---|---|
| **D1** | `var/generated/codex/model-catalogs/qwenzhai-models.json` | `var/generated/codex/config.toml` | Keeps `model-catalogs/` subdir (legacy Codex CLI convention) |
| **D2** | `var/generated/codex/qwenzhai-models.json` | `var/generated/codex/config.toml` | Flat layout; `model-catalogs/` was only needed for CODEX_HOME, which is now client-local |
| **D3** | `var/generated/codex/qwenzhai-models.json` | keep at `var/codex-home/config.toml` | Splits the pair; breaks coupling; not recommended |

**Decision: D2**

```
old A2:  var/codex-home/model-catalogs/qwenzhai-models.json
new A2:  var/generated/codex/qwenzhai-models.json

old A3:  var/codex-home/config.toml
new A3:  var/generated/codex/config.toml
```

**Rationale:**
- `model-catalogs/` subdirectory was a Codex CLI CODEX_HOME convention. `var/codex-home/` was a server-local CODEX_HOME before #58. After #58, CODEX_HOME is client-local and `var/codex-home/` has no CODEX_HOME role. The subdirectory adds no clarity.
- Both artifacts under `var/generated/codex/` is clean, parallel to `var/generated/model-inventory.json`.
- D3 (split pair) is rejected — it breaks the coupling and requires extra logic.

---

#### 4. Helper changes for D-impl

**Current `qz_paths.py` helpers:**

```python
codex_home_dir()            -> qz_var_dir() / "codex-home"         # stale concept
codex_model_catalog_dir()   -> codex_home_dir() / "model-catalogs"  # stale concept
codex_model_catalog_path()  -> codex_model_catalog_dir() / "qwenzhai-models.json"
codex_config_path()         -> codex_home_dir() / "config.toml"
```

**Changes for D-impl (H2-simplified):**

```python
# New helper — the server-side generated Codex artifact directory
def codex_generated_dir() -> Path:
    return qz_var_dir() / "generated" / "codex"

# Changed — now routes through codex_generated_dir(), not codex_home_dir()
...
# Kept — return value unchanged, concept deprecated
# Server no longer uses this as a writable Codex home; CODEX_HOME is client-local
def codex_home_dir() -> Path:
    return qz_var_dir() / "codex-home"

# Kept — return value unchanged, concept deprecated
# No runtime code calls this after D-impl; only tests and deprecated callers
def codex_model_catalog_dir() -> Path:
    return codex_home_dir() / "model-catalogs"
```

**Caller changes:** None. All runtime callers use `codex_model_catalog_path()` and `codex_config_path()` directly — they auto-follow the helper change.

---

#### 5. Server/client boundary

After #58, the boundary is:

```
Server                                     Client (qz-codex)
------                                     -----------------
var/generated/codex/qwenzhai-models.json   $HOME/.qz-codex/codex-home/model-catalogs/...
    ^-- written by generate()                  ^-- downloaded from /qz/codex/model-catalog
    ^-- served by /qz/codex/model-catalog
    ^-- metadata at /qz/codex/client-config
var/generated/codex/config.toml           $HOME/.qz-codex/codex-home/config.toml
    ^-- patched by generate()                  ^-- written by qz-codex-common launcher
```

**Rule:** Moving A2/A3 requires no qz-codex-common changes. The client always fetches catalog via HTTP and writes to its own local CODEX_HOME. `/qz/codex/model-catalog` remains the stable HTTP boundary; its URL does not change.

---

#### 6. Compatibility/shim strategy

**No symlink shim.** After #58, no clients read server A2/A3 directly.

**No old-path deletion.** Old files in `var/codex-home/` remain until the user cleans them up manually or a future slice adds a sweep. Leaving stale old files is harmless — they are not read.

**No dual write.** No server-side consumer reads the old paths after D-impl.

**Transition note for operators:** After D-impl, running `POST /qz/models/refresh` regenerates A2/A3 at the new paths. Old `var/codex-home/model-catalogs/qwenzhai-models.json` and `var/codex-home/config.toml` become orphaned but harmless.

---

#### 7. Staleness warning plan

Warning names are **unchanged**:

| Warning | Artifact | Staleness condition | After D-impl |
|---|---|---|---|
| `stale_model_inventory_cache` | A1 | A1 mtime < newest override mtime | Unchanged — A1 path already migrated |
| `stale_codex_catalog` | A2 | A2 mtime < A1 mtime | Path updates through helper; name/logic unchanged |
| `stale_codex_config` | A3 | A3 mtime < A2 mtime | Path updates through helper; name/logic unchanged |

`qz_config_report.py` reads A2/A3 paths through `_codex_model_catalog_path()` and `_codex_config_path()`. When helpers change, `effective_config_payload()` automatically reports and compares the new paths. No logic changes needed in the config report.

---

#### 8. `_refresh_codex_catalog()` behaviour after D-impl

```python
def _refresh_codex_catalog(self, catalog) -> bool:
    inventory_path = Path(os.environ.get("QZ_MODEL_INVENTORY_CACHE", str(_model_inventory_path())))
    catalog_dst = _codex_model_catalog_path()      # <- now returns var/generated/codex/qwenzhai-models.json
    config_dst = _codex_config_path()              # <- now returns var/generated/codex/config.toml
    catalog_dst.parent.mkdir(parents=True, exist_ok=True)   # creates var/generated/codex/ if needed
    generate_codex_catalog(inventory_path, catalog_dst, config_dst)
```

`catalog_dst.parent.mkdir(parents=True, exist_ok=True)` already creates the parent directory — no change to production code required beyond the helpers.

---

#### 9. `qz_codex_catalog.generate()` interaction
...
`generate()` always writes the absolute path of `catalog_dst` into `config_dst`. After D-impl, with both paths under `var/generated/codex/`, `config.toml` will contain:

```toml
model_catalog_json = "/path/to/var/generated/codex/qwenzhai-models.json"
```

No change to `generate()` logic needed.

---

#### 10. Tests for D-impl

New tests required (test_qz_paths.py):

```text
test_codex_generated_dir_is_under_var_generated
  codex_generated_dir() == qz_var_dir() / "generated" / "codex"

test_codex_model_catalog_path_moves_to_var_generated_codex
  codex_model_catalog_path() == codex_generated_dir() / "qwenzhai-models.json"

test_codex_config_path_moves_to_var_generated_codex
  codex_config_path() == codex_generated_dir() / "config.toml"

test_codex_generated_dir_follows_qz_var_dir_override
  QZ_VAR_DIR=/custom -> codex_generated_dir() == /custom/generated/codex

test_codex_generated_dir_does_not_create_directory
  calling codex_generated_dir() does not create directories

test_codex_home_dir_unchanged_after_d_impl
  codex_home_dir() still returns qz_var_dir() / "codex-home" (deprecated, not moved)

test_old_var_codex_home_catalog_not_created_by_default
  calling helpers does not create var/codex-home/model-catalogs/
```

Updated tests required (test_qz_paths.py):

```text
test_codex_model_catalog_path_default_matches_current_var_path
  expected = codex_generated_dir() / "qwenzhai-models.json"  (was codex_model_catalog_dir() / ...)

test_codex_config_path_default_matches_current_var_path
  expected = codex_generated_dir() / "config.toml"  (was codex_home_dir() / "config.toml")

test_qz_var_dir_env_override_is_respected
  codex_model_catalog_path() == custom_var / "generated" / "codex" / "qwenzhai-models.json"
  codex_config_path() == custom_var / "generated" / "codex" / "config.toml"

QzPathsCodexHomeIndependenceTests
  test_codex_model_catalog_path_ignores_codex_home — path under generated/codex, not client-home
  test_codex_config_path_ignores_codex_home — path under generated/codex, not client-home
  test_codex_home_dir_uses_var_dir_not_codex_home — unchanged (deprecated, still codex-home)
```

New tests required (test_qz_config_report.py):

```text
test_config_effective_reports_new_codex_catalog_path
  model_inventory_cache default = var/generated/codex/qwenzhai-models.json (wrong name in test — codex_model_catalog)

test_stale_codex_catalog_warning_works_with_new_path
  A2 at var/generated/codex/qwenzhai-models.json — stale warning fires

test_stale_codex_config_warning_works_with_new_path
  A3 at var/generated/codex/config.toml — stale warning fires
```

New tests required (test_qz_codex_client_config.py):

```text
test_model_catalog_path_uses_new_generated_path
  codex_model_catalog_content() reads from var/generated/codex/ path
```

---

#### 11. Non-goals for D-impl

```text
- Do not move A1 (already at var/generated/model-inventory.json)
- Do not change any script: qz-env, qz-codex-common, qz-up, qz-down
- Do not change CODEX_HOME semantics or qz-codex client behaviour
- Do not change /qz/codex/model-catalog URL or response shape
- Do not delete old var/codex-home/ files
- Do not add symlink shims
- Do not change model routing, profile slugs, or proxy behaviour
- Do not touch BrainCase, #37, or #51/#46
- Do not close #56
```
...
- One-line revert in `qz_paths.py`: restore `codex_model_catalog_path()` and `codex_config_path()` to old definitions, remove `codex_generated_dir()`
- Old files in `var/codex-home/` remain and are immediately usable again
- Regenerate via `POST /qz/models/refresh`
- No env override mechanism exists for A2/A3 (unlike A1's `QZ_MODEL_INVENTORY_CACHE`)
