...
```text
Prefer shlex.split() for simple commands.
...
| `test_warn_once_then_allow` | second repeat after warning allows |
...
| Model genuinely wants to re-read | First repeat gets signal; second repeat allowed |
...
4. A repeated read after prior warning is allowed through.
5. apply_patch, web_search, unknown tools, and dropped-tool errors keep existing behaviour.
6. repeated_read_signal telemetry appears in /qz/telemetry/request.
...
3. Add repeated_read_signal telemetry type.
4. Thread RepeatedReadState into completed_call_decision() with no behaviour change.
5. Enable signal-over-error-path for repeated reads.
6. Add decision tests.
7. Add stream/non-stream seeded-history tests.
8. Run full suite.
9. Live smoke against a known redundant-read prompt.
```
