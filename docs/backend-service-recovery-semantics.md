...
| `#6` | Backend VRAM telemetry — slices 1–4: service/recovery monitors, VRAM snapshot, process isolation, TurboQuant model-param probing; slice 5: live curses VRAM component panel (VRAM+COMP rows), BASE/DELTA removed; slice 6: provenance VRAM estimator; slice 6 follow-up 1: architecture-aware KV metadata; slice 6 follow-up 2: runtime cache budget preferred, quant registry; slice 6 follow-up 3: MODEL_RUNTIME calibrated from measured backend process VRAM minus KV_ALLOC (confidence=calibrated-from-host-observed-baseline/runtime), MODEL_FILE retained as non-subtractive provenance component (confidence=estimated-from-gguf-size), consistency adds calibrated status |
