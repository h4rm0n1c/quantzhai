...
def _probe_backend_process(container: str, timeout: float = 1.5) -> dict:
    """Try to isolate backend process VRAM via nvidia-smi compute-apps + docker top."""
    result = {
        "container": container,
    ...
    try:
        ...
        top_proc = subprocess.run(
            docker_cmd + ["top", container, "pid"],
            capture_output=True, text=True, timeout=timeout, check=False,
        )
        pids: set[str] = set()
        lines = top_proc.stdout.splitlines()
        ...
        for line in apps_proc.stdout.splitlines():
            ...
            if len(parts) >= 2 and parts[0] in pids:
                total += max(0.0, _safe_float(parts[1]))
                found = True

        if found:
            result["process_used_mib"] = total
            result["source"] = "nvidia-smi-compute-apps"
            result["confidence"] = "host-observed"
    except Exception:
        pass
    return result
...
def _probe_backend_metrics(timeout: float = 1.0) -> dict[str, float]:
    """Fetch llama.cpp /metrics and parse Prometheus text. Returns {} on failure."""
    ...
    try:
        with urllib.request.urlopen(url, timeout=timeout) as resp:
            if resp.status != 200:
                return {}
            return _parse_prometheus_text(resp.read().decode("utf-8", errors="replace"))
    except Exception:
        return {}
...
def build_vram_snapshot(handler=None, *, now: float | None = None) -> dict:
    ...
    try:
        ...
        metrics      = _probe_backend_metrics(timeout=1.0)
        context      = _build_context(handler=handler, metrics=metrics)
        return _assemble_snapshot(gpus, backend_proc, metrics, context, now=now)
