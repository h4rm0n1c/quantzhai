...
class ReasoningPolicyTests(unittest.TestCase):
    def test_prompt_policy_injects_sampling_and_removes_budget(self):
        ...
        self.assertIn("Use up to 8 tool calls", out["instructions"])
        self.assertIn("Read the most relevant files first", out["instructions"])
        self.assertIn("Cross-reference at least two sources", out["instructions"])
        self.assertIn("answer from the evidence gathered", out["instructions"])
